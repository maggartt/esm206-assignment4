...
##### Descriptive Statistics
<br>
<center>**Table 1.** Summary of Juvenile Hare Weights Across Sex</center>
```{r}
...
Assumptions of normalcy and homoscedasticity were tested using qqplot graphs and exploratory histograms. which appeared to be fully normal for further statistical analysis to be valuable. The mean weight of male juvenile hares is `r round(juvenile_hares_summary$mean[2], 1)` grams and that of female juvenile hares is `r round(juvenile_hares_summary$mean[1], 1)` grams, which results in a difference of `r round(means_difference, 1)` grams. Considering the standard deviation in male hares weight is `r round(juvenile_hares_summary$sd[2], 1)` grams and `r round(juvenile_hares_summary$sd[1], 1)` grams for females, this difference appears negligible. This is corroborated by the small effect size (Cohen's *d* = `r round(juvenile_cohen$estimate, 2)`), but the two-sample t-test suggests there is a significant difference in the sample means (two-sample t-test, t = `r round(juvenile_t$statistic, 2)`, p = `r round(juvenile_t$p.value, 3)`).
...
The visualization shows a simple linear regression assessing the relationship between weight (g) and hindfoot length (mm). The result is a slight linear relationship. This linear regression shows that hindfoot length predicts snowshoe hare weight (p < 0.001, R^2 = `r round(juvenile_hare_lm_glance$r.squared, 2)`) with an average slope of $\beta$ = `r round(juvenile_hare_lm_tidy$estimate[2], 1)` g mm^-1^. This means that with one millimeter increase in hindfoot length, we expect an average increase in hare weight of `r round(juvenile_hare_lm_tidy$estimate[2], 1)` grams. According to this linear regression, hindfoot length and hare weight are significantly positively correlated (Pearson's *r* = `r round(juvenile_hare_cor_tidy$estimate, 2)`, p < 0.001). In this case, the linear regression ignores if homoscedasticity is normal. However, as hindfoot length increases there appears to be increased variance in standard deviation. This implies that the data is heteroscedastic and therefore a simple linear regression model does not fully and accurately reflect the relationship between weight and hindfoot length. 
